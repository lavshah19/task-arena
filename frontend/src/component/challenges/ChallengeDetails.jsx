...
import { getTimeFromNow, passedDueDate } from "../../utils/dateUtils";
...
const ChallengeDetails = ({
  ...
  const isNotPastDue =
    dayjs().isBefore(dayjs(challenge.dueDate)) ||
    dayjs().isSame(dayjs(challenge.dueDate));

  const isPastDue = dayjs().isAfter(dayjs(challenge.dueDate));

  const timeRemaining = getTimeFromNow(challenge.dueDate);
  ...
  const copyPrivateChallengeLink = () => {
    const link = `${window.location.origin}/join-private/${challenge.inviteCode}`;
    navigator.clipboard
      .writeText(link)
      .then(() => {
        ...
        setTimeout(() => {
          setCopying(false);
        }, 1000); // Reset copying state after 1 seconds
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-md border border-gray-100 col-span-1 transition-all hover:shadow-lg">
      ...
      <div className="flex justify-between items-start mb-4">
        ...
        <span
          className={`px-3 py-1 rounded-full text-xs font-medium flex items-center gap-1 ${getStatusBadgeStyle()}`}
        >
          <Clock size={14} />
          {challenge.status}
        </span>
      ...
      <div className="flex items-center gap-2 mb-4 bg-gray-50 p-2 rounded-lg">
        ...
        <span className="text-sm font-medium text-gray-700">
          Created by:{" "}
          <span className="text-blue-600">{challenge.creator?.username}</span>
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div className="flex items-center gap-2 text-gray-600">
          ...
          <div>
            ...
            <p className="text-sm">
              {dayjs(challenge.dueDate).format("MMMM D, YYYY")}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 text-gray-600">
          <Clock
            size={18}
            className={isPastDue ? "text-red-500" : "text-blue-600"}
          />
          <div>
          ...
          <div className="flex items-center gap-1 bg-green-100 px-3 py-1 rounded-full">
            ...
            {isCreator && (
             <div className="flex items-center gap-1 ml-2">
              {
                copying ? ( <CopyCheck size={16} className="text-green-700" /> ) : (  
                  <Copy
                    size={16}
                    className="text-green-700 cursor-pointer"
                    onClick={copyPrivateChallengeLink}
                  />
                )
              }
              </div>
      ...
      >
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 mb-6">
            Are you sure you want to finish this challenge? This will finalize
            the challenge and award points to participants.
          </p>
